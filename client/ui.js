...
newProjectFormElement.addEventListener("submit", function (event) {
...
function initDragAndDrop() {
  ...
  function onDragEnd(e) {
    ...
    dragged = null;
    document
      .querySelectorAll(".dnd-container")
...
function initAnimalFilter() {
  ...
  const searchElement = document.getElementById("animal-search");
  const speciesElement = document.getElementById("animal-species");
  ...
  function onFilterChange() {
    renderList(searchElement.value, speciesElement.value);
  }

  searchElement.addEventListener("input", onFilterChange);
  speciesElement.addEventListener("change", onFilterChange);
...
window.addEventListener("load", () => {
  ...
  try {
    ...
    /* ignore for older pages */
  }
});

function initMapSection() {
  const animalsList = ["cat", "dog", "parrot", "shark", "ant"];
  const outElement = document.getElementById("map-output");
  const runBtnElement = document.getElementById("map-run");
  const upperBtnElement = document.getElementById("map-upper");

  function renderList(list) {
    outElement.replaceChildren();
    ...
    outElement.appendChild(frag);
  }

  runBtnElement.addEventListener("click", () => renderList(animalsList));
  upperBtnElement.addEventListener("click", () =>
    renderList(animalsList.map((n) => n.toUpperCase()))
  );

  // initial render
  renderList(animalsList);
}
